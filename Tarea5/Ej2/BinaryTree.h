...
class BinaryTree {
    ...
    public:
        ...
        void reflect(BNode<T> * node, BNode<T> *);
    ...
    template <class T>
    void BinaryTree<T>::reflect(BNode<T> * node,BNode<T> * raiz)
    {
        if (node) 
        {
            if (node->getRight())
            {
                BNode<int> * derecho = new BNode<int>(node->getRight()->getInfo());
                raiz->setLeft(derecho);
            }
            if (node->getLeft())
            {
                BNode<int> * izquierdo = new BNode<int>(node->getLeft()->getInfo());
                raiz->setRight(izquierdo);
            }
            
            reflect(node->getRight(),raiz->getLeft());
            reflect(node->getLeft(),raiz->getRight());
        }
    }